...
\]
The function $g$ has enough ordinary derivatives for Euler--Maclaurin; if
needed, subtract the next endpoint term, of order $x^{\alpha+2}$, before
applying the formula.  For the singular model, the power-sum expansion is
\[
...
\begin{proof}
We first dispose of the polar endpoint rectangles, on which the ratio $q/p$
used below need not be bounded.  If either index is within a fixed distance
of a pole and the two scales are comparable, both indices range over a fixed
finite set.  In north-polar coordinates
\[
 s=1-\frac{x}{M^2},\qquad t=1-\frac{y}{M^2},
\]
the variables $x,y$ range over fixed compact intervals and the chordal
distance is $M^{-1}$ times a uniformly bounded function of $x,y,\theta$
(with the harmless $O(M^{-2})$ correction absorbed uniformly).  Hence
$F_\alpha=M^{-\alpha}K_{\alpha,M}(x,y)$ with
$K_{\alpha,M}$ uniformly bounded.  Since these block measures have bounded
total variation, every such block is $O_\alpha(M^{-\alpha})$, which is
\eqref{eq:comparableblock} for this finite set.  The south pole is identical.
We can therefore assume below that the comparable rectangles are nonpolar.
...
\begin{equation}\label{eq:hexpalpha}
 ...
 \begin{cases}
 x^\nu B_\alpha(x),&\alpha\ne1,\\
 x\log(1/x)B_1(x),&\alpha=1,
\end{cases}                                         \tag{5.4}
\end{equation}
where $H_\alpha$ and $B_\alpha$ are analytic for $|x|<c$ and their
derivatives through order four are bounded on every smaller fixed interval;
$B_\alpha(0)\ne0$.  This formulation retains all analytic powers of $x$.
At the resonant value $\alpha=1$ it also retains the subsequent
$x^m\log x$ terms through the analytic factor $B_1$.
...
right side below.  The branch remainder obeys the same bound by direct
differentiation of \eqref{eq:hexpalpha}.  Thus, off the diagonal,
\begin{equation}\label{eq:powerder}
 |\partial_s^2\partial_t^2F_\alpha(s,t)|
 \le C_\alpha R^{-2-\alpha}|s-t|^{\alpha-3}.         \tag{5.5}
\end{equation}
...
\begin{equation}\label{eq:polyseriesderivative}
 ...
 \le C_\alpha(1+m)^4
 A^{\alpha/2-i-j}\bigl(Q^{m-2}+Q^m\bigr).           \tag{5.6}
\end{equation}
...
the same bound separately.  Since the polynomial factor in $m$ is summable
against $Q^m$ uniformly for $Q\le(1-\delta)^2$, taking $i=j=2$ gives
