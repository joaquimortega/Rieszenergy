...
\]
which settles the sum-of-distances problem for this configuration.  The
upper bound is uniform under independent rotations of the polygons on the
occupied parallels.
\end{abstract}
\maketitle
...
\]
For later use, the elementary population arithmetic is
\begin{align*}
 \sum_{j=1}^{M-1}(4j-1)
 &=4\frac{(M-1)M}{2}-(M-1)
   =(M-1)(2M-1),\\
 r_M&=N-2(M-1)(2M-1).
\end{align*}
Since $M=\lfloor\sqrt{N/4}\rfloor$,
\[
 4M^2\le N<4(M+1)^2,
\]
and hence
\[
 6M-2\le r_M\le14M+1.
\]
Thus the central population is positive and of order $M$.  After shared
boundary atoms are combined, symmetry gives the exact cardinality
\[
 \sum_{j=1}^{2M-1}r_j
 =2(M-1)(2M-1)+r_M=N.
\]
...
For $j<M$ the sums can be evaluated explicitly:
\[
 \sum_{k=1}^jr_k=j(2j+1),\qquad
 H_j=1-\frac{2j(2j+1)}N,\qquad
 h_j=1-\frac{4j^2-2j+1}{N}.
\]
Consequently $1-H_j\asymp j^2/M^2$.  Since
$1+H_j$ stays between two positive constants in a northern polar block,
\[
 \rho_j^2=1-H_j^2=(1-H_j)(1+H_j)\asymp\frac{j^2}{M^2},
\]
so $\rho_j\asymp j/M$ and $w_j\asymp j$.  Reflection gives the same
estimates in the southern hemisphere; the preceding bounds on $r_M$ absorb
the central exceptions.  Summing the resulting radius profile yields
$\sum_p\rho_p\asymp M$, proving \eqref{eq:geometry}.
...
\begin{align*}
 E_\alpha(\cP_N)
 &=\sum_p\Sigma_{\alpha,p}^{\rm self}
   +\sum_{p\ne q}\Sigma_{\alpha,pq}^{\rm cross},\\
 E_\alpha[\nu]
 &=\sum_pw_p^2\rho_p^\alpha J_\alpha
   +\sum_{p\ne q}w_pw_qF_\alpha(z_p,z_q),
\end{align*}
where both sums use ordered pairs.  Therefore
\begin{align*}
 I_\alpha N^2-E_\alpha(\cP_N)
={}&I_\alpha N^2-E_\alpha[\nu]\\
 &+\sum_p\left(w_p^2\rho_p^\alpha J_\alpha
                    -\Sigma_{\alpha,p}^{\rm self}\right)\\
 &+\sum_{p\ne q}\left(w_pw_qF_\alpha(z_p,z_q)
                    -\Sigma_{\alpha,pq}^{\rm cross}\right),
\end{align*}
which is exactly \eqref{eq:decomp}.
...
\]
because $\nu(\Sph)=N$ and
$\int|x-y|^\alpha d\sigma(y)=I_\alpha$.  Conditional negative
definiteness therefore gives $A_{\alpha,N}\ge0$; the identical computation
with the atomic point measure gives $D_{\alpha,N}\ge0$.
...
\]
Indeed, after fixing the first vertex, the other vertex has each nonzero
cyclic difference $k=1,\ldots,w-1$ exactly once, and there are $w$ choices
of the first vertex.  The continuous angular self-energy on the same ring is
$\rho^\alpha w^2J_\alpha$.  Hence
\begin{align*}
 &\rho^\alpha w^2J_\alpha-\Sigma_{\alpha}^{\rm self}\\
 &\quad=\rho^\alpha w\left[
 wJ_\alpha-\sum_{k=1}^{w-1}
 \left(2\sin\frac{\pi k}{w}\right)^\alpha\right].
\end{align*}
Lemma~\ref{lem:circle} now shows that this deficit is
\begin{equation}\label{eq:ringdef}
 -2(2\pi)^\alpha\zeta(-\alpha)
 \rho^\alpha w^{1-\alpha}+o_\alpha
 (\rho^\alpha w^{1-\alpha}).                         \tag{3.2}
\end{equation}
The leading coefficient is positive because $\zeta(-\alpha)<0$ for
$0<\alpha<2$.  Exact nonnegativity for every population follows from
conditional negative definiteness on the circle.
The uniform part of Lemma~\ref{lem:circle} bounds the absolute ring deficit
by $C_\alpha\rho^\alpha w^{1-\alpha}$.  The two-sided comparison
$cM\rho_p\le w_p\le CM\rho_p$ implies, for either sign of $1-\alpha$,
\[
 \rho_p^\alpha w_p^{1-\alpha}
 =\rho_p\left(\frac{w_p}{\rho_p}\right)^{1-\alpha}
 \le C_\alpha M^{1-\alpha}\rho_p.
\]
Using \eqref{eq:geometry} and adding the already separated $w_p=1$ rings,
\[
 0\le B_{\alpha,N}
 \le C_\alpha M^{1-\alpha}\sum_p\rho_p
 =O_\alpha(M^{2-\alpha})
 =O_\alpha(N^{1-\alpha/2}).
\]

We now compute the square-subsequence coefficient.  Along $N=4m^2$ one has
$M=m$.  For a northern ordinary band,
\[
 r_j=4j-1,\qquad
 h_j=1-\frac{4j^2-2j+1}{4m^2}.
\]
Because $\widetilde r_j=r_j+O(1)$ and
$\operatorname{rem}_j=O(1)$, its midpoint population is
\[
 w_j^{\rm mid}
 =\frac{4\widetilde r_j}{6}+\operatorname{rem}_j
 =\frac23r_j+O(1)=\frac83j+O(1).
\]
The shared boundary between bands $j$ and $j+1$ has population
\[
 w_j^{\rm bdry}
 =\frac{\widetilde r_j+\widetilde r_{j+1}}6
 =\frac{r_j+r_{j+1}}6+O(1)
 =\frac43j+O(1).
\]
Thus, with $x=j/m$, the midpoint and shared-boundary populations satisfy
\[
...
while
\[
 h_j\longrightarrow1-x^2,\qquad
 \rho_j=\sqrt{1-h_j^2}\longrightarrow
 \sqrt{1-(1-x^2)^2}=x\sqrt{2-x^2}.
\]
The bounded remainder in
...
\]
For the last integral, $u=2-x^2$ gives
\[
 \int_0^1x(2-x^2)^{\alpha/2}\,dx
 =\frac12\int_1^2u^{\alpha/2}\,du
 =\frac{2^{1+\alpha/2}-1}{\alpha+2}.
\]
Write the product of the bracket and this integral as $K_\alpha$.  The
Riemann sum says
$m^{\alpha-2}\sum_p\rho_p^\alpha w_p^{1-\alpha}\to2K_\alpha$.
Combining it with \eqref{eq:ringdef} and
\[
 N^{1-\alpha/2}
 =(4m^2)^{1-\alpha/2}=2^{2-\alpha}m^{2-\alpha}
\]
gives
\[
 \frac{B_{\alpha,4m^2}}{(4m^2)^{1-\alpha/2}}
 \longrightarrow
 -\frac{4(2\pi)^\alpha}{2^{2-\alpha}}
 K_\alpha\zeta(-\alpha)
 =-K_\alpha(4\pi)^\alpha\zeta(-\alpha).
\]
Thus $K_\alpha=\kappa_\alpha$ in the statement.
For $\alpha=1$ one has
...
the $L$-grid occurs $d$ times among the $qr=dL$ ordered pairs.  To see the
multiplicity, write $q=dq'$, $r=dr'$ with $(q',r')=1$.  The difference map
\[
 (\ell,k)\longmapsto \frac{\ell}{q}-\frac{k}{r}\pmod1
\]
has image the grid $L^{-1}\mathbb Z/\mathbb Z$; its domain has $dL$
elements and every fiber has the same cardinality, hence cardinality $d$.
Thus the discrete pair average is exactly a shifted $L$-point trapezoidal
average.  Multiplying its normalized error by $qr$ and using
...
\]
The last equality is the explicit calculation
\[
 qrL^{-1-\alpha}
 =qr\left(\frac{d}{qr}\right)^{1+\alpha}
 =\frac{d^{1+\alpha}}{(qr)^\alpha}.
\]
...
factor $M^{-\alpha}$ can be expanded as
\begin{align*}
 &\sum_{q,r\le CM}
 \frac{(q,r)^{1+\alpha}}{(qr)^{\alpha/2}}\\
 &\quad=\sum_{a\le CM}J_{1+\alpha}(a)
 \sum_{\substack{q,r\le CM\\a\mid q,\ a\mid r}}
 (qr)^{-\alpha/2}\\
 &\quad=\sum_{a\le CM}J_{1+\alpha}(a)a^{-\alpha}
 \left(\sum_{u\le CM/a}u^{-\alpha/2}\right)^2.
\end{align*}
Because $0<\alpha/2<1$,
\[
 \sum_{u\le X}u^{-\alpha/2}\le C_\alpha X^{1-\alpha/2}.
\]
Using this and $J_{1+\alpha}(a)\le a^{1+\alpha}$, the last display is at
most
\begin{align*}
 ...
 &\le C_\alpha M^2.
\end{align*}
Here the final step is
$\sum_{a\le CM}a^{\alpha-1}\le C_\alpha M^\alpha$.
Restoring the prefactor $M^{-\alpha}$ gives
$C_\alpha M^{2-\alpha}$.
...
\[
 ...
 \le C_\alpha M^{2-\alpha}.
\]
Indeed, the summand before summation is
\[
 \left(\frac rM\right)^{\alpha/2}
 \frac{r^{1+\alpha}}{(Mr)^\alpha}
 =M^{-3\alpha/2}r^{1+\alpha/2}.
\]
