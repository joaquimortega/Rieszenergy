...
def main() -> None:
    ...
    # Lean includes `env.mainModule` in automatically generated private proof
    # names. Preserve the standalone module identity for its final header,
    # while elaborating each source body under its original module identity.
    chunks.append(
        "\nrun_cmd do\n"
        "  let original := (← Lean.getEnv).mainModule\n"
        "  Lean.Elab.Command.modifyScope fun s =>\n"
        "    { s with opts := s.opts.setName `bemoc.export.originalMainModule original }\n"
    )
    for module in ordered:
        ...
        chunks.append(
            f"\nrun_cmd Lean.modifyEnv (·.setMainModule `{module})\n"
    ...
    chunks.append(
        "\nrun_cmd do\n"
        "  let original := (← Lean.getOptions).getName `bemoc.export.originalMainModule .anonymous\n"
        "  Lean.modifyEnv (·.setMainModule original)\n"
        "  Lean.Elab.Command.modifyScope fun s =>\n"
        "    { s with opts := s.opts.erase `bemoc.export.originalMainModule }\n"
    )
