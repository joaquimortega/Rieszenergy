# CapLowerBound proof guide

This module proves the universal Beck lower bound for the actual cap discrepancy. The primary mathematical source is Bilyk and Brauchart, [On the lower bounds for the spherical cap discrepancy](https://arxiv.org/html/2502.15984), Section 2. The formal proof obtains the explicit constant `1/16` for the manuscript's ordinary `dt` normalization. It works for repeated point labels, so it implies the `BeckLowerBound` contract with its injectivity premise.

The geometric input is the exact projection moment of normalized surface area: even moments of degree `2r` equal `1/(2r+1)`, while odd moments vanish. Tensor features express every integer power of the spherical inner product as a finite sum of products, proving positivity of power-moment gaps. Diagonal terms then show the normalized even gap is at least `1/(2n)` for degrees `2r` with `r ≥ n`.

The analytic input is the positive binomial coefficient block from `BinomialTail`. Rather than require the binomial identity at the endpoints of the inner-product interval, the proof expands `sqrt(2) * sqrt(1-ρ⟪x,y⟫)` for `0 ≤ ρ < 1`. Its partial kernels have a uniform summable majorant, so their pair energies converge by dominated convergence. Positivity lets the proof retain the finite block `r ∈ [n,2n)`. A second dominated-convergence step sends `ρ` to `1`, where the kernel is chordal distance. The checked Stolarsky identity turns the resulting distance-energy deficit into the squared cap discrepancy; elementary square-root and real-power algebra yields `beckLowerBound`.

Status: complete for `BeckLowerBound`. `lake build BEMOCFormalization.CapLowerBound` passes without custom axioms, `sorry`, `admit`, or `opaque` shortcuts.

## Exact dependencies and normalization

The only project imports are `CapDiscrepancy` and `BinomialTail`. Through
them the proof uses the normalized sphere probability measure, its uniform
height marginal, reflection invariance, the constant distance potential,
and the proved Stolarsky identity. The binomial module supplies the real
square-root series and an explicit positive coefficient block. No estimate
for the Diamond construction is used. The theorem is universal for every
`X : Fin n → Sphere` with n>0.

All finite energies count ordered pairs, including the diagonal. Surface
area is a probability measure, while the cap discrepancy integrates ordinary
dt on [−1,1]. These conventions give
`n² I₁ − E₁(X) = 4n² D(X)²`. The factor four is retained in the final
constant calculation.

## Projection moments and positivity

For a unit vector u, reflection invariance moves u to the north pole.
The uniform height marginal then gives
`∫⟨u,x⟩^(2r) dσ(x)=1/(2r+1)` and zero for odd powers. Let μ be the
counting measure of X and ν=nσ. Both have mass n. The power-kernel mixed
energy of μ and ν equals the self-energy of ν because each projection
moment is independent of its axis.

For every natural m, expand the kernel as
`⟨x,y⟩^m = Σ_p Φ_p(x) Φ_p(y)`, where p ranges over maps from an m-element
set to the three coordinates and Φ_p is the product of the chosen
coordinates. This is a finite sum. Integrating against the difference of
the two finite measures gives a sum of squares of feature-integral
differences. Thus every power-moment gap
`G_m=E_m(μ,μ)−E_m(ν,ν)` is nonnegative, including odd degrees. The proof
uses ordinary pair integrals of positive finite measures to express this
identity, so it does not rely on an implicit signed-measure integration API.

For m=2r, each summand in the finite configuration energy is nonnegative,
and the n diagonal summands each equal one. Hence
`G_(2r) ≥ n−n²/(2r+1) ≥ n/2` whenever r≥n. Coincident labels increase
some off-diagonal terms and do not invalidate this argument. This explains
why injectivity is unnecessary in the stronger underlying theorem.

## Positive coefficient block and the two limits

Let `b_m=−(-1)^m choose(1/2,m)` for m≥1. These coefficients are positive.
For 0≤ρ<1, expand
`Kρ(x,y)=sqrt(2) sqrt(1−ρ⟨x,y⟩)` at an argument of absolute value below
one. The constant coefficient cancels between ν and μ because their
masses agree. The regularized distance deficit is therefore the limit of
positive sums `sqrt(2) Σ_m b_m ρ^m G_m`.

The finite partial-sum argument keeps only even indices m=2r with
n≤r<2n. Every discarded term is nonnegative; the retained gaps are at
least n/2. Fixed-ρ binomial convergence has a summable majorant independent
of x,y, so integration commutes with this first limit. It yields

`deficit(Kρ) ≥ sqrt(2) (n/2) Σ_(n≤r<2n) b_(2r) ρ^(2r)`.

The second limit uses ρ=k/(k+1). The actual regularized kernels are bounded
by two uniformly on the sphere product, and the measures are finite.
Dominated convergence gives pair-energy convergence as ρ tends to one.
The right side is a finite polynomial in ρ and converges directly. At
ρ=1, the kernel equals chordal distance by the unit-sphere identity
`dist(x,y)²=2−2⟨x,y⟩`. Endpoint binomial convergence is therefore not
silently assumed during either integration step.

`BinomialTail` proves `Σ_(n≤r<2n) b_(2r) ≥ 1/(32 sqrt(n))`. Combining
this with the retained gap n/2 gives
`n² I₁−E₁(X) ≥ sqrt(2) sqrt(n)/64 ≥ sqrt(n)/64`.
Stolarsky now yields `D(X)²≥1/(256 n sqrt(n))`. Positivity of n and the
real-power identities identify the right side as
