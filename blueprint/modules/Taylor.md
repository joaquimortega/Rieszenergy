...
The abstract `TwoMomentRule` assumes additivity for every function. The concrete interval integral defining `bandError` is totalized on nonintegrable functions, so it must not be installed as a `TwoMomentRule` without restricting its domain. The concrete proof below uses `bandError_add_of_integrable` and establishes integrability from the local smoothness hypotheses at each use.
