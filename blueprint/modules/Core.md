...
`continuousEnergy α` is the exact scalar `2^(α+1)/(α+2)`. It is a *definition*, so the integral identity still needs proof. Under `0<α<2`, the denominator is positive. For the first-variable potential, rotate a fixed point to the north pole, where the kernel is `(2−2t)^(α/2)`. Integrate against `dt/2`; substitution `u=2−2t` gives the stated constant. The existing `continuousEnergy_one` is only a numerical specialization, useful when discussing discrepancy, not evidence for the integral formula.
...
Status: `parallelVector_mem_sphere` and `continuousEnergy_one` are proved. The surface measure interpretation, chordal identity, kernel-average identity, and energy integral interpretation remain proof work. Their absence must not be hidden behind the fact that the current file elaborates.
