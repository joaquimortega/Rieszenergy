# Comparable near-tail scaling

The near angular tail has pointwise scale `R^(α−8) θ^(α−4)` after four height derivatives. Integrating θ from `1/r` contributes `r^(3−α)` and leaves exactly `M^8/(r^5 M^α)`. The module proves these as real-power identities without hiding the `r^α` factor before integration.
