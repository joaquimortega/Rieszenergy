# SobolevMomentSpectral: actual finite spectral moment estimate

`SobolevMomentSpectral.lean` proves the universal Sobolev cubature lower bound for the actual spectral norm. Its final theorem is `sobolevOptimality Y hs1 hs2 : SobolevOptimality Y s` for every harmonic basis and `1 < s < 2`. The central estimate is `evenMomentSpectralBound Y hs0 hs2 : EvenMomentSpectralBound Y s` for `0 ≤ s ≤ 2`, with explicit constant `3136`. No spectral estimate or embedding is assumed in the final theorem.

The proof first uses the Fischer decomposition and arbitrary-basis finite span theorem to reconstruct the centered degree-`2N` polynomial exactly from harmonics of degrees at most `2N`. The all-degree Funk–Hecke action in `SobolevMomentAction.lean` identifies every positive even harmonic coefficient as `evenMomentModelCoefficient N k` times the corresponding quadrature coefficient. Odd and degree-zero coefficients vanish. This gives a finite spectral norm and an exact finite expansion of the quadrature gap. The positive weighted coefficient inequality from `SobolevLowerBound.lean` then bounds the norm by `3136 N^(s−1)` times that gap, degree by degree.

The checked `SobolevEmbeddingDirect.sobolevEmbedding_of_harmonicAddition` establishes embedding directly from the harmonic addition diagonal and a convergent scalar `p`-series when `s>1`. Combining it with the moment estimate and `sobolev_optimality_of_even_moment_spectral_bound` proves the final theorem without a distance-kernel expansion.
