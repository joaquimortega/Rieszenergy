# LatitudePotential proof guide

Source anchor: the constant-potential cancellation in `definitive.tex` around lines 606–612. This module proves the exact height identity needed to cancel the mixed terms in the latitude error:

`half_intervalIntegral_latitudeKernel {s α} (hs : s ∈ Set.Icc (-1) 1) (hα : 0 < α) : (1/2) * ∫ t in (-1)..1, latitudeKernel α s t = continuousEnergy α`.

For fixed `s`, `parallelMeasure s hs` is uniform angular measure pushed to the spherical parallel. Every spherical point is a `parallelPoint` at its height and some angle, including the poles. The chordal distance formula identifies the distance-power angular average with the defining `latitudeKernel`; periodicity removes the chosen angle. Swapping the parallel and surface integrals then makes the inner surface potential equal to `continuousEnergy α`, by `constantPotential_of_pos`. Finally `hasUniformHeightMarginal` converts the surface integral of the latitude kernel into its height integral. No full height-angle disintegration of `sigma` is needed for this identity.

The helper `continuous_latitudeKernel` establishes joint continuity for positive exponent, which supports the pushforward integral. The proof is independent of the discrete construction and carries the exact condition `0 < α`.

Status: complete. `lake build BEMOCFormalization.LatitudePotential` passes; the proof-shortcut audit finds no `sorry`, `admit`, `axiom`, or `opaque`, and `git diff --check` is clean.
