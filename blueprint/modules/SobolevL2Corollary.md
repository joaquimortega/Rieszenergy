# SobolevL2Corollary: cubature on genuine L² classes

This module formulates the manuscript's Sobolev worst-case error on the actual `Lp ℝ 2 sigma` spectral unit ball. A raw L² equivalence class has no point values. For `s>1`, `l2SobolevRepresentative` chooses the unique continuous representative proved in `SobolevL2.lean`; `l2SobolevWCE` quantifies over **every** L² unit-ball class and evaluates only that representative. The chosen value is independent of the proof of unit-ball membership by uniqueness.

`l2SobolevWCE_eq` proves equality with the verified continuous spectral WCE. `l2_sobolev_corollary`, `l2_sobolev_optimality`, and `l2_sobolev_energy_comparison` transfer the Diamond upper bound, universal lower bound, and energy comparison to the L² formulation; `l2_sobolev_targets` packages all three. The module checks coefficient, termwise norm, and full squared norm equality between each L² class and its representative. The underlying Fourier coefficients and norm are the actual L² quantities from `HarmonicL2.lean` and `SobolevL2.lean`.
