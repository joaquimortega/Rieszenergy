# SeparatedAngularExpansion: the angular integral as an even power series

This module turns the latitude kernel's angular integral into a series with only even modes. It imports `KernelDerivatives` for `latitudeKernel` and `SeparatedBinomialSeries` for convergence of the generalized binomial coefficients. The algebraic factors `angularKernelA = 2 - 2st` and `angularKernelB = 2√(1-s²)√(1-t²)` separate the angular cosine from the heights.

`summable_restricted_angularBinomialTerm_norm` bounds each continuous binomial term on the compact full-circle interval by `|Ring.choose β n| |q|ⁿ`. This provides the normal convergence required by mathlib's interval-integral/`tsum` interchange. `tsum_angularBinomialTerm_eq_rpow` then evaluates the pointwise binomial series, so `normalized_angular_rpow_eq_tsum` gives the normalized average as a sum of cosine moments. Translating the second half-circle by π negates every odd cosine power; the odd moments vanish. The moment bound `|normalizedCosineMoment n| ≤ 1` gives summability for the even/odd split, yielding `normalized_angular_rpow_eq_even_tsum`.

The final theorem, `latitudeKernel_eq_separatedEvenPowerSeries`, factors the positive base `A` from the integrand and rewrites the even powers of `B` as `4^m(1-s²)^m(1-t²)^m`. Its explicit hypotheses are physical heights, `A>0`, and the local ratio `|B/A|<1`. It proves equality with the actual angular integral at those points; a uniform geometric ratio bound is supplied by later modules.
...
/-! ## The polynomial even expansion on the closed height square -/
...
/-- The radius factors in the even mode have natural exponents, so the
formula remains meaningful at both poles. -/
noncomputable def separatedEvenPowerSummand
    (α : ℝ) (m : ℕ) (s t : ℝ) : ℝ :=
...
instances of it. -/
theorem latitudeKernel_eq_separatedEvenPowerSeries
    ...
    latitudeKernel α s t =
      ∑' m : ℕ,
        (Ring.choose (α / 2) (2 * m) *
          ...
          separatedEvenPowerSummand α m s t := by
