# SphereProjection proof guide

The normalized surface measure has a uniform height marginal. Its first absolute height moment is `1/2`, and its signed height moment is zero. A Householder reflection sends any unit direction to the north pole while preserving `sigma`, transferring these height moments to the projection in an arbitrary unit direction. Scaling gives the result for every ambient vector, including zero.

Public declarations: `sphere_projection_abs_integral` proves `∫ u : Sphere, |inner (u : Ambient) v| ∂sigma = ‖v‖ / 2`; `sphere_projection_integral` proves the signed projection integral is zero. The statements are unconditional and use the geometric measure verified in `SurfaceMeasure`.

Status: complete. `lake env lean BEMOCFormalization/SphereProjection.lean` elaborates without proof shortcuts.
