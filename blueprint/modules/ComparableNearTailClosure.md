# Comparable near-tail closure

`BEMOCFormalization.ComparableNearTailClosure` proves the previously isolated
`NearTailDerivativeBound α` for every `0 < α < 2`, and then combines it with
the small-angle estimate to prove `near_comparable_block_bound`. The exact
source regime is nonpolar comparable bands with zero-based index gap at most
two and global size `1024 ≤ N`. No assumption is made on the relative
phases of their circular point sets.

The proof first controls the integrand at every height pair in the product
of closed bands and every angle in `[r_j⁻¹, π]`. The height chain bound is
obtained from the polar derivative table at orders `(2,2)`, `(2,1)`,
`(1,2)`, and `(1,1)`. The target-band radius may be a factor eight smaller
than the source-band radius. Consequently the shared lower radius is
`ρ = r_j/(240M)`; using `r_j/(30M)` for both coordinates would be false.
The upper chord bound and lower chord estimate imply

`|mixedFourth(latitudeProfile α · · θ)| ≤ K_α ((r_j/M)θ)^(α-4) / ρ^4`.

The exponent is negative, and the lower chord estimate must therefore be
used in the reverse direction for the inverse power. The explicit factor
`28800²` accounts for this reversal. `near_tail_profile_theta_bound`
records the resulting pointwise inequality without hiding the constants
behind asymptotic notation. The supplied polar derivative constant is
uniform over all `N`, all source and target indices, and all angles.

The generic `tail_integral_le_power_majorant` proves an absolute-value
interval-integral bound from a pointwise power majorant. It proves the
relevant almost-everywhere inequality on the restricted interval measure,
checks interval integrability of the positive power majorant, and invokes
mathlib's norm bound for interval integrals. This avoids assuming absolute
integrability of the derivative integrand independently. The scalar estimate
`truncated_angular_power_integral_bound` gives the integral of
`θ^(α-4)` from `r_j⁻¹` to `π`. Since `α<2`, the exponent is below `-2`,
and the integral is bounded by `r_j^(3-α)/(3-α)`. In particular the
calculation remains valid at `α=1` and has no logarithmic resonance.

For the smoothness witness, set `W = nearTailPhysicalRegularSet r_j`. The
construction in `ComparableNearBlocks` proves that `W` is open, contains
the whole closed band rectangle, lies in the open physical height square,
and has positive chord for every angle in the truncated interval. The
generic parameter-integral theorem in `NearTailIntegralCalculus` proves
`ContDiffOn ℝ 4 (nearTailKernel α r_j) W` and moves the mixed fourth
derivative through the angular integral at every rectangle point. These
facts are valid even when source and target bands overlap, because the
excluded interval of angles starts at the strictly positive `r_j⁻¹`.

The normalization is delicate. The pointwise scaling identity is
`((r_j/M)θ)^(α-4) / ((r_j/M)/240)^4 =
240^4 (r_j/M)^(α-8) θ^(α-4)`. After integration, the exact cancellation
`r_j^(3-α) (r_j/M)^(α-8) = M^8/(r_j^5 M^α)` supplies the target fourth
derivative scale. The factor `r_j^α` within the pointwise power is essential;
dropping it would give the wrong population exponent. The final constant is
positive because `C_α>0`, `π>0`, and `3-α>0`.

`near_comparable_block_bound` invokes the compiled low-angle bound and the
new smooth-tail contract. The final block estimate uses the signed band
measure, the exact low/tail decomposition, and the two-moment Taylor bound
for the C4 tail. The factor `(1+|j-k|)^(α-3)` is recovered from
`|j-k|≤2` with a fixed constant. This module can be imported by the full
comparable assembly; the endpoint and intermediate-distance regimes are
proved in separate modules.
