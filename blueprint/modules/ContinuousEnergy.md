...
Implementation status: these interfaces are now inhabited externally by `measureNegativeType_of_pos_of_lt_two` in NegativeType, `constantPotential_of_pos` in SurfaceMeasure, and `energy_nonnegative` in EnergyDecomposition. `SurfaceMeasure` also proves that sigma is a probability measure with uniform height marginal. The full `SurfaceIntegration` height-angle formula is still a separate obligation.
