# ComparableIntermediateBlocks proof guide

Source anchor: `definitive.tex`, the regular intermediate comparable case `3≤|j−k|≤2r_j` around lines 1937–1993.

`intermediate_comparable_profile` combines checked radius comparison and polar-angle separation with population comparability. For `R=r_j/M`, `d=|j−k|/M`, it gives radii between `R/30` and `R` on the first band and between `R/240` and `8R` on the second, plus angle gap between `d/30` and `30d`. `intermediate_comparable_chord_bounds` then uses the two-sided chord comparison and `d≤2R` to prove the pointwise model `d²/3600+R²θ²/30000≤D²≤4096R²` for `0≤θ≤π` on each literal rectangle. These constants suffice for the order-four angular inverse-power integral and for absorbing lower derivative orders. `intermediate_inverse_chord_le` applies the negative-exponent monotonicity to majorize the actual chord power by the precise model `((d/60)²+((R/180)θ)²)^((α−4)/2)`, matching the checked scalar integral theorem. `intermediate_inverse_chord_integral_bound` proves the actual angular integral inequality: the chord stays positive for every angle because its zero-angle lower bound is positive, so both powers are continuous and interval-integrable; interval monotonicity and `angular_inverse_power_integral_bound` give `(1+1/(3−α))(d/60)^(α−3)/(R/180)`.

Status: the intermediate geometry is fully checked. The generic compact-interval calculus is now specialized to the full half-turn profile: `intermediate_profile_smooth_extension` constructs a genuine `C⁴` extension equal to the latitude kernel on the literal rectangle, and `intermediate_mixedFourth_eq_integral` passes the mixed fourth height derivative under the angular integral exactly. `ComparableIntermediateClosure` supplies the quantitative polar-to-height derivative bound, Taylor estimate, and exact comparable majorant. `lake env lean BEMOCFormalization/ComparableIntermediateBlocks.lean` passes.
