# SeparatedSmoothExtension: one function across polar boundaries

This module imports `SeparatedSeriesSum` and `ScalarPowerSeries`. `separatedOpen` requires a positive algebraic base and an absolute series ratio below one. It is open, includes physical points with positive base and `separatedRatio<1`, and can contain points on both sides of the polar boundary. `separatedRatio_mem_unit_of_mem_open` supplies the scalar power series' convergence-domain condition.

`separatedSmoothExtension` defines one global-plane function as `A^(α/2)` times the scalar series in `separatedRatio`; no piecewise choice of a physical versus exterior function is made. It equals `separatedEvenSeries` when `A>0`, and `separatedSmoothExtension_eq_latitudeKernel` identifies it with the actual latitude kernel at physical points in `separatedOpen`. That last proof uses `B²=4(1-s²)(1-t²)` to turn the algebraic ratio bound into the angular binomial bound.

`separatedSmoothExtension_contDiffOn` proves `C⁴` regularity on `separatedOpen` for `0<α<2`: the base real power is smooth where positive, the rational ratio is smooth where its denominator is nonzero, and the bounded-coefficient scalar power series is `C⁴` on `(-1,1)`. This gives the regularity needed for boundary transfer and the later mixed-derivative identification. It does not by itself provide a numerical mixed-fourth bound.
