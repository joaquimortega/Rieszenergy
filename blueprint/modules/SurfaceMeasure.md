# SurfaceMeasure proof guide

The source anchors are `definitive.tex` equations `eq:integralaintervalo` and `eq:Falpha`, plus the constant-potential use after `eq:decomp`. This module proves the probability normalization of the geometric surface measure, its uniform height marginal, and `ConstantPotential α` for every `0 < α`.

The normalized sphere measure is the scalar normalization of mathlib's `volume.toSphere`; positivity and finiteness of its total mass give `sigma Set.univ = 1`. To establish the height marginal, the proof identifies the cone over the spherical height sublevel set with a solid region in the unit ball, computes its Euclidean volume by slicing, and compares the resulting CDF to the uniform law on `(-1,1]`. Equality of CDFs gives equality of measures. The north-pole potential is then a one-dimensional real-power integral under this height law. Reflection invariance of `volume.toSphere` carries the value from the north pole to every point of the sphere.

Public declarations: `sigma_apply_univ`, the `IsProbabilityMeasure sigma` instance, `hasUniformHeightMarginal`, and `constantPotential_of_pos`. The latter elaborates the exact `ConstantPotential` contract from `ContinuousEnergy`, with value `continuousEnergy α = 2^(α+1)/(α+2)`.

The full `SurfaceIntegration` height-angle contract is not proved in this module; the uniform height marginal suffices for the potential evaluation. This proof is independent of the discrete construction.

Status: complete for the stated claims. `lake env lean BEMOCFormalization/SurfaceMeasure.lean` elaborates, the shortcut audit finds no forbidden proof shortcuts, and `git diff --check` is clean.
