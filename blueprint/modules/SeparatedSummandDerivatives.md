# SeparatedSummandDerivatives: exact derivatives of one even mode

This module imports `SeparatedAngularExpansion` and works with its polynomial-radius summand `separatedEvenPowerSummand`. Positivity of `angularKernelA` gives four-times differentiability of the real-power base in each height; the radius powers have natural exponents and remain algebraically defined at the poles. `separatedEvenPowerSummandMixed22` records the actual iterated `(2,2)` derivative in mathlib's convention.

The `unequalRadiusPower*` and `unequalAPow*` definitions spell out first and second radius derivatives and every base derivative needed through total order four. `separatedEvenPowerSummandDS`, `DSS`, `DSST`, and `DSSTT` combine them by the product rule. The corresponding `HasDerivAt` lemmas establish the chain of two left and two right derivatives from these finite formulas; they do not appeal to formal symbolic differentiation alone.

`iteratedDeriv_two_separatedEvenPowerSummand_left` replaces the first two actual derivatives by `DSS` using local positivity and eventual equality of first derivatives. `separatedEvenPowerSummandMixed22_eq_DSSTT` repeats this argument in the right height and identifies the actual mixed derivative with the explicit expression. The result is pointwise for `angularKernelA s t>0`; summing these derivatives requires the normal bounds proved later.
