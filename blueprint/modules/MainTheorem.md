...
Checked result: `main_theorem` proves `MainTheorem α` unconditionally for `0 < α < 2`, uniformly over every `N ≥ 4` and every phase choice; `main_theorem_target` packages the full exponent range. `main_theorem_of_latitude_longitude` remains a reusable conditional assembly. Its inputs are now discharged by `latitude_bound_of_blocks` and `longitude_bound`, while `diamond_nonnegative` and the phase-uniform `small_size_bound` cover nonnegativity and the finite-size cutoff. The proof architecture below records those dependencies.
...
The proof of `LongitudeBound α` applies the phase-uniform trapezoid lemma to `G(θ)=(A−B cos θ)^(α/2)` with `A≥B≥0`. For rings `j,k`, the multiset of differences between the `r_j` and `r_k` polygon angles consists of `L=lcm(r_j,r_k)` equally spaced angles, each with multiplicity `d=gcd(r_j,r_k)`. Thus the ordered cross-ring error is bounded by `C_α M^{-α} d^(1+α)/(r_jr_k)^(α/2)`, using `r_j≍Mρ_j`. Population multiplicity at most three and `r_j≤15M` reduce the total to `gcdSum α (15M)`. Write `u=da`, `v=db`; the resulting sum is `O_α(M²)` because `α>0` makes the relevant double p-series convergent. This proves `O(M^(2−α))`, uniformly in all phase differences. It includes within-ring diagonal pairs without a special exclusion.

For `SmallSizeBound α`, the proof handles each of the finitely many integers `4≤N<1024` with a *phase-independent* estimate. Under `α>0`, every pair distance on the unit sphere is at most `2`, so `0≤diamondEnergy≤2^α N²`, and `deficit≤I_αN²`. The scale is strictly positive for these `N`; choose one explicit constant from the endpoint bound, or take a finite maximum over integer `N`. This avoids invoking compactness of the phase space. `DiamondNonnegative α` comes from the measure-level negative-type result and exact cardinality, as detailed in ContinuousEnergy and EnergyDecomposition.

`main_theorem_target` fixes `α`, obtains all analytic inputs for `0<α<2`, and applies the assembled bound. The zero-phase `diamondPoints` are covered as a special case because the public theorem quantifies over all phases; `diamondPoints_card` and `diamondPoints_energy` connect that convention to the manuscript's finite set. No `sorry`, `admit`, custom axiom, or opaque shortcut is used.

`main_theorem_of_comparable` is the last conditional assembly: for
0<α<2 its sole analytic input is `ComparableBlockBound α`. The two unequal
regimes are supplied by the proved `sameSideBlockBound` and
`oppositeBlockBound` from `UnequalBlockEstimates`, which use the global
separated smooth extension and its quantitative mixed fourth derivative.
The unconditional `main_theorem` supplies this final input by `comparable_block_bound`; earlier assembly lemmas remain available with their explicit inputs.
...
regimes, using the checked identity, symmetry, and finite summation. They remain conditional assembly theorems, while `comparable_block_bound`, `sameSideBlockBound`, and `oppositeBlockBound` discharge their premises in `main_theorem`.
