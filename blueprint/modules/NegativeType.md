# NegativeType proof guide

The source anchor is the conditional negative definiteness argument following `eq:decomp` in `definitive.tex`. This module proves `MeasureNegativeType α` for `0 < α < 2` and does not use the surface measure or its normalization.

The proof expands the Gaussian kernel `exp(-t‖x-y‖²)` as a positive scalar multiple of `exp(2t⟨x,y⟩)`. Truncated exponential series are positive definite because each tensor monomial has a squared moment for the difference of the two measures. Dominated convergence passes the inequality to the Gaussian kernel. The equal-mass hypothesis makes `1-exp(-t‖x-y‖²)` conditionally negative. Schoenberg's positive integral representation of `r^(α/2)` integrates these inequalities over `t>0`; compactness of the sphere and finiteness of the measures supply the integrability needed for Fubini. Finally, kernel symmetry converts the two mixed terms into the exact `2 *` term in `MeasureNegativeType`.

The public theorem is `measureNegativeType_of_pos_of_lt_two`. Its assumptions are exactly `0 < α` and `α < 2`; finite measures and equal masses remain explicit in the contract. The proof does not depend on any construction-specific objects from the legacy project.

Status: complete. `lake env lean BEMOCFormalization/NegativeType.lean` elaborates, the shortcut audit finds no `sorry`, `admit`, `axiom`, or `opaque`, and `git diff --check` is clean.
