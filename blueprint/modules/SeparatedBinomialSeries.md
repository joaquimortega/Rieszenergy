# Generalized real binomial series

This module proves the real binomial theorem for every real exponent β and
every real x with |x|<1. It supplies both convergence and the exact value
of the series, together with polynomially weighted convergence for later
derivative estimates. All statements below have Lean proof bodies.

## Dependencies and reuse

The only imports are Mathlib's smooth-series differentiation, finite-dimensional
normed modules, real-power differentiation, elementary limits, and generalized
binomial coefficients. There is no dependency on the Diamond construction or
on a latitude estimate. The proof is adapted from the old development's
`RealBinomialSeries` module; it is generic real analysis and transfers without
any assumption about the earlier Simpson configuration.

`BinomialTail` uses these results for the positive coefficients of the
square-root expansion in the Beck lower bound. `SeparatedAngularExpansion`
uses them to expand the averaged latitude kernel at strictly separated
heights. The latter application uses a strict geometric ratio below one.

## Coefficient recurrence and absolute convergence

Write bₙ=choose(β,n). The polynomial binomial identity gives
`(n+1)b_(n+1)=(β−n)b_n`. Dividing by n+1 gives a recurrence that remains
valid when a coefficient vanishes. No division by bₙ occurs, so nonnegative
integer exponents, whose series terminate, are included.

The absolute recurrence multiplier |(β−n)/(n+1)| tends to one. Given
|x|<1, choose |x|<r<1. Eventually the norm of the next series term is at
most r times the current norm, and Mathlib's ratio majorization criterion
proves summability. For any fixed natural d, the extra multiplier
`((n+2)/(n+1))^d` also tends to one. The same proof yields summability
after multiplying each term by `(n+1)^d`; finite derivative losses can
therefore be absorbed without changing the disk of convergence.

## Termwise differentiation

Define Tₙ(x)=bₙxⁿ and T'ₙ(x)=n bₙx^(n−1). The n=0 formula is harmless
because its scalar coefficient is zero. The identity
`(n+1)choose(β,n+1)=β choose(β−1,n)` rewrites the derivative series in
terms of the series with exponent β−1.

For a fixed interior point choose |x|<ρ<1. On the connected open interval
(−ρ,ρ), the derivatives have the summable uniform majorant
`|β choose(β−1,n−1) ρ^(n−1)|`, with the zeroth term set to zero.
Convergence at the base point zero and this derivative majorant are the
hypotheses of `hasDerivAt_tsum_of_isPreconnected`. Consequently the sum Sβ
is differentiable and `Sβ'=β S_(β−1)` throughout (−1,1). All infinite-sum
shifts and extractions of scalar factors have explicit summability proofs.

## Identification of the sum

Pascal's identity, followed by separating the zeroth term, gives
`Sβ(x)=(1+x)S_(β−1)(x)`. Since 1+x>0 on the interval, the preceding
derivative identity becomes `Sβ'=β Sβ/(1+x)`. Also Sβ(0)=1.

Instead of invoking uniqueness of an abstract differential equation, the
proof differentiates `Sβ(x)(1+x)^(-β)` directly. Its derivative vanishes
on the connected interval, so its value equals its value at zero, namely
one. Positivity of the real-power base then gives the final theorem
`tsum_real_choose_mul_pow_eq_rpow`:

`Σₙ choose(β,n)xⁿ=(1+x)^β` whenever |x|<1.

This module makes no endpoint convergence assertion at x=±1. The special
square-root endpoint needed for cap discrepancy is proved separately in
`BinomialTail`; the lower-bound proof can also pass to the endpoint through
interior regularization. The finite-order polynomial summability theorem is
an input for subsequent derivative arguments, not by itself a statement
that an arbitrary parameterized series can be differentiated.
