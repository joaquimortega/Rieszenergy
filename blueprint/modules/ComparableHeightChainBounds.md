# Comparable height-chain bounds

`BEMOCFormalization.ComparableHeightChainBounds` converts the uniform polar
angular derivative estimates into a bound for the mixed fourth derivative
of the physical latitude profile. The polar table is supplied by
`PolarDerivatives.polar_derivative_bound` for every `0 < α < 2`. This module
performs the coordinate change `s=cos φ`, `t=cos ψ` with
`φ=arccos s`, `ψ=arccos t` only at interior physical heights and positive
chord. Both restrictions are substantive: derivatives of `arccos` diverge
at the poles, and the noninteger Riesz power is not C4 at zero chord.

The local regularity interface starts with `polarPositiveSet α θ`, the
open subset of polar-angle pairs at which the squared chord is positive.
`polarKernel_contDiffOn_positive` proves C4 regularity there. For an
individual pair `(φ,ψ)` with positive chord,
`polar_height_chain_regular` chooses an open vertical polar-angle slice,
proves both component functions are C2 on that slice, and supplies the
local hypotheses of the generic chain identity from `ComparableNearBlocks`.
This is deliberately local. The profile need not be C2 globally as a
function of either polar angle, because it may have a diagonal singularity
outside the chosen slice.

`abs_polar_height_mixedFourth_le` applies the factored chain identity and
the four polar jet bounds. The terms correspond exactly to jet orders
`(2,2)`, `(2,1)`, `(1,2)`, and `(1,1)`. The denominators include
`a²b²`, `a³b²`, `a²b³`, and `a³b³`, where
`a=sqrt(1-s²)`, `b=sqrt(1-t²)`. If both radii are at least `ρ`, the
chord is at most `(Lρ)²`, and `L≥1`, the lower polar orders can be
replaced by the common inverse chord power `U^((α-4)/2)`. The resulting
bound is `C(1+2L+L²) U^((α-4)/2)/ρ⁴`. This domination is valid because
`U>0` and the upper chord bound converts positive powers of U into
powers of `Lρ`. The coefficient estimates retain the explicit condition
`|s|,|t|≤1`, inherited from the physical height domain.

The statement actually needed downstream concerns
`latitudeProfile α s t θ`, which is defined using heights and square
roots rather than the polar kernel. `latitudeProfile_eq_polarKernel_arccos`
proves equality at physical heights using the trigonometric identities for
`arccos`. Pointwise equality is insufficient to transfer four derivatives:
`mixedFourth_congr_of_rect_eqOn` obtains equality of fourth derivatives
from equality on a neighborhood rectangle. Interior heights provide such
an open rectangle, so
`abs_latitudeProfile_mixedFourth_le_of_positive` exports the quantitative
bound directly for the latitude profile. This wrapper prevents later
modules from repeating local C4 and congruence arguments.

`near_tail_profile_chord_bound` specializes the wrapper to comparable
nonpolar bands with index separation at most two. The source population
is `r_j` and the band parameter is `M`. Although the source radius is at
least `r_j/(30M)`, comparability only implies the target population is
at least `r_j/8`; the common radius is therefore `ρ=r_j/(240M)`.
The global near-band chord upper bound permits `L=12000`. These deliberately
loose constants make the estimate uniform in the exceptional central
geometry and in all point phases. They depend on neither `N` nor indices.

Finally, `inverse_chord_power_le` applies the lower chord comparison
`D²/28800≤U`, where `D=(r_j/M)θ`. Because `α-4<0`, the inequality for
the rpow reverses, giving `U^((α-4)/2)≤28800² D^(α-4)`.
The power `28800²` is slightly larger than necessary for some α but is
uniform across `0<α<2`. This lemma is reused in the near angular tail
closure; intermediate-distance blocks use the same height-chain wrapper
with a different positive chord model and their own angular integral.
