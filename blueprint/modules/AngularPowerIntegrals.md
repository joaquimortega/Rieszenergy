# Angular power integrals proof guide

`AngularPowerIntegrals.lean` proves the scalar angular integrals needed for comparable and near block estimates. For an exponent `0 < α < 2`, the inverse-power kernel is bounded on `[0, b]` by splitting at the length scale `δ`: the first part is at most `δ^(α−3)`, and the tail is at most `δ^(α−3)/(3−α)`. Scaling the angular variable by `R` yields the factor `R⁻¹`. The reciprocal-scale tail estimate follows by directly integrating `θ^(α−4)` between `r⁻¹` and `π`.

Public declarations: `angular_inverse_power_integral_bound` and its expanded-square and unscaled variants; `truncated_angular_power_integral_bound`; `truncated_angular_power_integral_nonneg`. All bounds are explicit and unconditional under their stated scalar hypotheses. The module contains no custom axioms or deferred proof steps.

Status: complete. `lake build BEMOCFormalization.AngularPowerIntegrals` passed.
