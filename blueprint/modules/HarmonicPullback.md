# Harmonic pullback proof guide

`HarmonicPullback.lean` constructs the algebraic substitution of a three-variable polynomial by an ambient orthogonal linear equivalence. Each substituted coordinate is the linear polynomial with coefficients given by the columns of the transformation. Evaluation therefore agrees with evaluation at the transformed ambient point, and Mathlib's homogeneous-substitution theorem preserves the declared degree.

The proof differentiates the substitution algebraically. The resulting polynomial chain rule has one coordinate-matrix coefficient for each derivative. Applying it twice and summing over the derivative coordinate produces the matrix product `T Tᵀ`; orthogonality collapses its coefficients to the Kronecker delta, yielding `polynomialLaplacian_orthogonalPolynomialPullback`. Consequently `orthogonalHarmonicPullback` is a linear endomorphism of the actual homogeneous harmonic submodule. Its restriction to the sphere is pointwise composition with `sphereLinearIsometryEquiv T`.

Status: complete. `lake build BEMOCFormalization.HarmonicPullback` passed. No custom axioms or deferred proof steps occur in the module.
