# HarmonicLaplacian: intrinsic polynomial sphere Laplacian

`PolynomialSphere` is the range of the actual polynomial restriction map in continuous sphere functions. For each coordinate plane, `sphereAngularDerivation` descends the ambient rotation derivation to this range. Well-definedness is proved geometrically: if a polynomial vanishes on the sphere, its orbit under every genuine plane rotation is identically zero, so the derivative at zero—the angular derivation—vanishes there too. `sphereAngularDerivation_hasDerivAt` identifies the descended operator with this derivative of the rotation pullback for every polynomial sphere function.

`spherePolynomialLaplacian` sums the squares of the three tangential rotation generators. It agrees with the polynomial Casimir on every restriction and acts on a degree-`ℓ` homogeneous harmonic restriction by `−ℓ(ℓ+1)`. This module establishes the differential operator and the forward eigenfunction bridge. The separate weak `L²` module supplies the full eigenspace converse using self-adjointness and harmonic completeness; no arbitrary smooth-domain converse is asserted here.
