...
The module imports `KernelDerivatives` for the common kernel framework and
Mathlib for generalized binomial coefficients and geometric-series estimates.
It imports no legacy module. `ScalarPowerSeries` uses its polynomial-geometric
summability result; the developing separated derivative modules use all of
its algebraic estimates. The statements below are proved for closed height
intervals, so physical poles are included.

## Separation ratio and endpoint-safe bounds

Put U=2−2st, u=1−s² and v=1−t². For heights in [−1,1], u and v are
nonnegative, each is at most U, and U≤4. The inequalities follow by expanding
(s−t)² and using the nonnegativity of the other squared radius.
The polynomial ratio is Q=4uv/U². If U>0 and
`2 sqrt(u) sqrt(v) ≤ (1−ε)U`, with 0<ε<1, squaring nonnegative quantities
proves `0≤Q≤(1−ε)²<1`. This calculation does not divide by either radius.

After at most two derivatives in each height have hit the radius factors,
a tail term contains `4^m u^(m−a) v^(m−b)`, with m≥2 and a,b≤2. The exact
factorization

`4^m u^(m−a) v^(m−b) = 16 (4uv)^(m−2) u^(2−a) v^(2−b)`

keeps every radius exponent a natural number. Replacing the last two radius
factors by their U bounds yields a power `U^(4−a−b)`. This is valid even
when one radius vanishes; no negative power of a radius is introduced.
The eventual derivative series can therefore be majorized by a geometric
factor Q^(m−2) times a polynomial in m and a power of U.

## Summability and binomial coefficients

For every natural d and 0≤q<1, `summable_separated_polynomial` proves
summability of `(m+1)^d q^m`. The case q=0 is treated by finite support.
For q>0, shifting the standard polynomial-geometric series and dividing by
q gives the result. This generic lemma also controls every finite derivative
of a scalar series whose original coefficients have absolute value at most
one.

For 0≤β≤1, the recurrence
`(n+1)choose(β,n+1)=(β−n)choose(β,n)` and the bound |β−n|≤n+1 prove
`|choose(β,n)|≤1` by induction. The proof only divides by the positive
integer n+1, so vanishing coefficients cause no exception.

These estimates provide majorants and algebraic identities. Identifying the
actual mixed fourth derivative with the bounded formal series requires the
separate termwise differentiation and smooth-extension arguments; importing
this module alone does not discharge that analytic obligation.
