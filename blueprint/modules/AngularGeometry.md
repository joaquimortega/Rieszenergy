# Angular geometry proof guide

Source anchors: `definitive.tex` `eq:geometry` and the latitude appendix's polar-angle estimates. This module proves the angular contract declared in `Geometry.lean` for `M≥16`. It imports the completed elementary `GeometryBounds` work.

The proof uses the mean value theorem for cosine on each occupied angular band. The drop in cosine is exactly the height width `2r_j/N`, so an interior sine converts height width to angular width. On northern ordinary bands, the explicit boundary formula gives `sin θ≤2j/M`. Away from the cap, the preceding boundary also gives `sin θ≥j/(4M)`. The cap's upper width uses Jordan's sine inequality, since the sine vanishes at the pole. At the equator, central height zero, `r_M≤15M`, and `M≥16` keep both boundary heights within `±1/2`, giving `sin θ≥1/2`. Reflection carries the northern estimates to southern bands.

The resulting width bounds are `1/(4M)≤Δθ_j≤15/M`. On noncap bands, `(1/30)r_j/M≤sin θ≤r_j/M`. Telescoping the widths across separated bands gives angular distance between `1/(30M)` and `30/M` times the label separation. Consequently `angular_geometry : AngularGeometry` is proved with `c=1/30` and `C=30`; all three conjuncts of the contract are discharged without new assumptions.

Status: proved in `BEMOCFormalization/AngularGeometry.lean`. The module must be imported transitively by `BEMOCFormalization.lean` when integrated.
