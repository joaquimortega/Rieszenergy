# Harmonic addition: degree-diagonal normalization

`BEMOCFormalization/HarmonicAddition.lean` defines the finite degree kernel
`harmonicAdditionKernel Y ℓ x y` for the actual `HarmonicBasis` observables.
For every basis and degree, its diagonal integrates to `2 * ℓ + 1` against
the normalized surface measure `sigma`.

The module constructs `harmonicBasisDegree Y ℓ`, an orthonormal basis of the
full finite-dimensional space of homogeneous harmonic polynomials from the
specified `Y.function ℓ k`. This uses the genuine sphere integral inner
product and the proved dimension `2 * ℓ + 1`.
`orthonormalBasis_sum_sq_linearMap` proves that the squared evaluations of
any linear functional have the same sum over any two orthonormal bases.
Consequently `harmonicAddition_diag_basis_independent` identifies pointwise
diagonals for any two `HarmonicBasis` values, and
`harmonicAddition_diag_of_orthonormal_family` works for any polynomial
orthonormal family of the full degree.

`harmonicAddition_rotated_family_orthonormal` applies the proved invariance
of `sigma` to a polynomial family whose restrictions are rotated degree
functions. `harmonicAddition_diag_rotated_of_family` packages this with
basis independence: such a family has the original pointwise diagonal.

`HarmonicPullback` proves that ambient orthogonal pullback preserves the
harmonic polynomial subspace and that its restriction composes with the
corresponding sphere isometry. The pullback of
`harmonicBasisPolynomial Y ℓ k` is another orthonormal family by invariance
of `sigma`. Basis independence proves
`harmonicAddition_rotationInvariant Y`; the diagonal average and sphere
transitivity then give the unconditional `harmonicAddition_diag Y ℓ x`, with
value exactly `2 * ℓ + 1`. Finite Cauchy–Schwarz gives the unconditional
`harmonicAddition_abs_le_dim Y ℓ x y`. Both theorems concern the actual
continuous functions in the specified `HarmonicBasis`.
