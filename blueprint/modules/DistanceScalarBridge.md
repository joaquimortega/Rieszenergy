# `BEMOCFormalization.DistanceScalarBridge` proof guide

This module closes the scalar Legendre series for the chordal distance kernel, conditional only on the exact harmonic addition identity `HarmonicAdditionLegendre Y`. It imports `DistanceScalarUniqueness` and the separately checked coefficient calculation in `DistanceZonalCoefficients`.

The affine substitution `t=1−2y` converts the normalized interval coefficient into `2^(2s−2) ∫₀¹ y^(s−1)P_ℓ(1−2y)dy`. The checked shifted Rodrigues/Beta computation gives, for `1<s<2` and `ℓ≥1`, half the chordal-power coefficient `−a_ℓ`. At degree zero it gives the exact mean `I_(2s−2)`.

To normalize Legendre orthogonality under the addition hypothesis, integrate the square of the basis addition kernel in its second variable. Orthonormality leaves `Σ_k Y_(ℓ,k)(x)^2=2ℓ+1`; spherical height is uniform on `[-1,1]`, so the addition identity yields `∫₋₁¹P_ℓ(t)^2dt=2/(2ℓ+1)`. Orthogonality to all other degrees follows directly from Rodrigues.

The positive coefficient series has a summable uniform majorant. Termwise integration and orthogonality therefore isolate each Legendre moment of `Σ_{ℓ≥1}a_ℓ(2ℓ+1)P_ℓ(t)`. The exact chordal coefficients show that these moments equal those of `I_(2s−2)−(2−2t)^(s−1)`. Both functions are continuous on the closed interval, and `DistanceScalarUniqueness` makes them equal pointwise. The final checked theorem `sobolevEnergyComparison_of_addition` transfers this genuine pointwise kernel identity to the independently defined Sobolev worst-case error. `harmonic_addition_legendre` now discharges the all-degree addition premise in the unconditional `Corollaries.sobolev_energy_comparison`.
