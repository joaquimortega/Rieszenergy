# ComparableSeparatedBlocks proof guide

Source anchor: `definitive.tex`, the opposite-pole endpoint and regular far cases of the proof of `eq:comparableblock` around lines 1924–1932 and 2022–2041.

`opposite_small_bands_taylor_bound` uses the global separated-series `C⁴` extension and its uniform fourth-derivative estimate. On a north-first-eight by south-last-eight rectangle, `U=2−2st≥1`, so the derivative is bounded by one constant. Taylor gives `C r_j³ r_k³/M⁸`. `opposite_small_bands_index_scale` proves `M≤1+|j−k|≤2M`, and the populations are at most 32. `endpoint_comparable_power_lower` and `opposite_endpoint_taylor_scale` convert this to the exact comparable majorant in `opposite_small_bands_comparable_bound`.

`far_comparable_taylor_bound` applies the same global extension to regular far rectangles. The preceding geometry theorem gives `U≥(|j−k|/M)²/3600` and fixed separation `V≤(1−1/4000000)U`, so Taylor gives a block estimate with factor `C((|j−k|/M)²/3600)^(α/2−4)`. `ComparablePowerScaling` converts it to `C_α r_j M^(−α)(1+|j−k|)^(α−3)` in `far_comparable_block_bound`. Both theorems are unconditional for `0<α<2` on their stated index classes. `reverse_opposite_small_bands_comparable_bound` handles the swapped south-north corner by kernel-block symmetry and the factor-eight population comparison.

The opposite-pole endpoint and regular far-comparable cases are checked here. Same-pole endpoints are in `ComparableBlockEstimates`; `ComparableNearTailClosure` and `ComparableIntermediateClosure` prove the remaining near and intermediate cases, and `ComparableAssembly.comparable_block_bound` performs the exhaustive assembly. `lake env lean BEMOCFormalization/ComparableSeparatedBlocks.lean` passes.
