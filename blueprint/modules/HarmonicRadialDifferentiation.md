# HarmonicRadialDifferentiation: Hobson identity

This module proves that a homogeneous harmonic polynomial `H` of degree `ℓ`, viewed as a constant-coefficient differential operator, acts on `radialSquare^a` by `2^ℓ * a.descFactorial ℓ * radialSquare^(a−ℓ) * H`. It also covers `a<ℓ`: the descending factorial and operator action are both zero.

The proof establishes the Weyl commutator for coordinate multiplication, lifts it from monomials to arbitrary polynomial operators, and obtains the radial commutator with a Laplacian term. Harmonicity removes that term. Induction on `a` applies the identity to `H` and its harmonic coordinate partials; Euler's homogeneous identity and the descending-factorial Pascal identity combine the coefficients. The operator is the concrete `polynomialDifferentialOperator` of `SobolevLowerBound`, so the theorem can feed the finite-integral moment bridge directly.
