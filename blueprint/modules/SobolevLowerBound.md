# SobolevLowerBound: a concrete universal moment test

This module works with the actual continuous-function `SobolevUnitBall`, spectral norm terms, quadrature error, and supremum from `Sobolev.lean`. Its continuous test `evenMomentTest X r` averages the functions `(x·X_i)^(2r)` over the nodes. By `sphere_projection_even_moment`, its integral is `1/(2r+1)`. The centered test subtracts this constant and has zero degree-zero Fourier coefficient. It is the restriction of `centeredEvenMomentPolynomial X r`, whose total degree is at most `2r`. At `r=n`, `CapLowerBound.even_projection_moment_gap_ge` proves that its quadrature error is at least `1/(2n)` for every `n`-point rule, including rules with coincident nodes.

The checked theorem `sobolev_optimality_of_even_moment_spectral_bound` proves the exact `SobolevOptimality Y s` target from `SobolevEmbedding Y s` and the explicit analytic statement `EvenMomentSpectralBound Y s`. This latter predicate bounds the **actual spectral norm squared of the specified moment test** by `A n^(s−1)` times its positive quadrature gap. The transfer rescales that test into the actual unit ball and obtains a constant times `n^(−s/2)`; it does not replace the worst-case error by a surrogate definition.

The coefficient estimates in this module give positivity, `≤1/n`, fourth-power decay, and the weighted bound `≤3136 n^(s−1) (1+(2k)(2k+1))^(−s)` for `0≤s≤2`. The module also proves the ambient projection moment formula, finite coefficientwise integration, and commutation with the polynomial differential operator. `SobolevMomentAction.lean` completes the all-degree harmonic action using the checked radial differentiation identity. `SobolevMomentSpectral.lean` then identifies the actual harmonic coefficients of the centered test, reconstructs its finite harmonic expansion, and proves `EvenMomentSpectralBound` unconditionally for `0≤s≤2` with `A=3136`.

The exact universal lower assertion is now proved in `SobolevMomentSpectral.lean`. Its final theorem uses the spectral bound here and the direct embedding in `SobolevEmbeddingDirect.lean`, without a conditional analytic hypothesis. The source motivation is [Brauchart–Saff–Sloan–Womersley, Theorem 3](https://arxiv.org/pdf/1208.3267). The manuscript's Sobolev optimality statement is in `definitive.tex`, lines 150–188.
