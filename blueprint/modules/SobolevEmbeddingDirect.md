# `BEMOCFormalization.SobolevEmbeddingDirect` proof guide

The actual spectral Sobolev unit ball embeds uniformly into continuous functions for every `s>1`, using only the unconditional harmonic addition diagonal. This proof is independent of the chordal-distance kernel expansion and therefore supplies the embedding input for the universal Sobolev lower bound before the energy comparison is closed.

The diagonal identity says `Σ_k Y_(ℓ,k)(x)^2=2ℓ+1` for every point. Hence the point-evaluation spectral term is exactly `(2ℓ+1)(1+ℓ(ℓ+1))^(−s)`, independent of `x`. For `ℓ≥1`, this is at most `3ℓ^(1−2s)`: the base `1+ℓ(ℓ+1)` dominates `ℓ²`, while `2ℓ+1≤3ℓ`. The p-series converges because `1−2s<−1`. The resulting total diagonal sum has a positive uniform upper bound `K`.

`SobolevKernel` already proves finite weighted Cauchy–Schwarz, uniform Cauchy convergence of the true harmonic partial sums under such an evaluation bound, and identification of the limit with the original continuous representative by basis completeness. Applying those checked lemmas gives `sobolevEmbedding_of_harmonicAddition`: every function in the independently defined spectral unit ball satisfies `|f(x)|≤√K` uniformly. No distance-expansion hypothesis enters this theorem.
