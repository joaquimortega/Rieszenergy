# `BEMOCFormalization.SobolevL2` proof guide

The manuscript defines its Sobolev space as an `L²(S²,σ)` space, while the earlier formalized worst-case error used continuous functions. This module proves that the two spectral unit balls are canonically the same for `s>1`: every `L²` unit-ball class has a unique continuous representative, and every continuous spectral unit-ball function defines such an `L²` class. The upstream `HarmonicL2` module defines the actual `Lp ℝ 2 sigma` Fourier coefficient and proves that these coefficients distinguish `L²` classes.

For any abstract degree-indexed harmonic coefficient sequence `a`, define its weighted degree term as `(1+ℓ(ℓ+1))^s Σ_k a_(ℓ,k)^2` and its finite synthesis as `Σ_(ℓ<L)Σ_k a_(ℓ,k)Y_(ℓ,k)`. Orthonormality gives the exact Fourier coefficient of each finite synthesis: it is `a_(m,j)` if `m<L`, and zero otherwise. Finite spectral Cauchy–Schwarz bounds the square of a difference of partial sums by the product of its Sobolev norm tail and the point-evaluation spectral tail.

`SobolevEmbeddingDirect` proves the evaluation spectrum is uniformly summable for `s>1`, using the harmonic diagonal `Σ_kY_(ℓ,k)(x)^2=2ℓ+1` and the convergent p-series `Σℓ^(1−2s)`. The Sobolev norm tail tends to zero by the explicit summability in `L2SobolevUnitBall`. These facts make the harmonic synthesis Cauchy in the sup norm, so it converges to a continuous function `g`. Continuity of Fourier extraction gives the prescribed coefficients for `g`.

`HarmonicL2.l2HarmonicCoefficient_injective` identifies `continuousToLp g` with the original `L²` class `f`, because all their Fourier coefficients agree. The canonical continuous map to `Lp` is almost everywhere equal to `g`, giving the required representative equality. The weighted terms and unit-ball inequality are identical coefficient by coefficient. A second continuous representative has the same `Lp` image, and `continuousToLp_injective` makes it equal to `g`. The checked theorem `l2Sobolev_continuousRepresentative` states this existence and uniqueness. The named lemmas `harmonicCoefficient_eq_l2_of_ae` and `sobolevNormTerm_eq_l2_of_ae` expose exact coefficient and norm preservation; `continuous_to_L2SobolevUnitBall` proves the converse inclusion needed for worst-case error equality.
