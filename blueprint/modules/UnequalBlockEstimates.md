# UnequalBlockEstimates: uniform rectangle geometry and block assembly

`UnequalBlockEstimates.lean` supplies the exact height geometry for eightfold unequal bands. The smaller ring lies in the polar half of a hemisphere and is never central. Same-side rectangles have positive radial term `U=2−2st`, angular coupling `V≤(255/256)U`, and scale lower bound `U≥(r_k/M)²/64`. Opposite or central rectangles have `U≥8/5` and the same fixed coupling gap. The proofs cover entire closed band rectangles and reflect southern configurations exactly.

A tensor band rule depends only on kernel values on its rectangle. The global separated-series extension agrees with the latitude kernel throughout every unequal rectangle and is `C⁴` there. Its uniform mixed-fourth derivative bound feeds `MixedTaylorBound`; the scale lemmas then prove the unconditional `SameSideBlockBound α` and `OppositeBlockBound α` for `0<α<2` with the manuscript's precise powers.
