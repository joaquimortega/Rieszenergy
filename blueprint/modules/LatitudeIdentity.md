# LatitudeIdentity: exact signed latitude decomposition

`LatitudeIdentity.lean` proves `latitudeIdentity_of_pos` for every positive exponent. It uses the height potential theorem `half_intervalIntegral_latitudeKernel` from `LatitudePotential.lean` and the ordered band rules from `BandErrors.lean`.

The occupied band integrals telescope from the south pole to the north pole. Summing the signed rules therefore gives the full height integral minus the weighted midpoint sum. The inner sum of kernel errors is continuous in the outer height. Its full height integral vanishes: the continuous potential is constant, the atomic heights lie in `[-1,1]`, and their populations sum to `N`. Applying the outer signed rule then leaves the negative atomic evaluation. Expanding that finite sum yields `latitudeError = -∑ kernelBlock`, with ordered pair normalization and the original variable binding.

The public theorem requires `0 < α`. The module uses the actual geometric potential theorem, not a conditional placeholder.
