# SeparatedBoundaryTransfer: polar endpoint continuity

`SeparatedBoundaryTransfer.lean` transfers an interior fourth-derivative estimate for the single global separated extension to physical polar endpoints. A point `(s,t)∈[-1,1]²` is approached by contracting both coordinates toward zero. The approximants lie in `(-1,1)²`, remain eventually in the open separated domain, and satisfy the interior ratio threshold `q'` whenever the endpoint ratio is at most `q<q'`.

The proof derives continuity of `mixedFourth` from the extension's `C⁴` regularity on the open domain, then passes the inequality to the limit. `separatedRatio_le_of_angular_gap` converts the geometric `V/U≤255/256` into a squared series-ratio threshold, leaving strict slack for the limit argument. This is the endpoint step needed by a uniform derivative bound proved first for strict interior heights.
