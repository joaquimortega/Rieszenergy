# `BEMOCFormalization.Angular.CuspSmoothing`
...
```lean
import BEMOCFormalization.Angular.CuspFourier
import BEMOCFormalization.Angular.Schoenberg
...
**Checked status.** This module builds after importing `Angular.CuspFourier` and `Angular.Schoenberg`. It adapts the legacy smoothing proof into the new isolated analytic namespace. It proves a uniform Fourier bound for every smoothing parameter `δ≥0` and the endpoint cusp `δ=0`; this is the bridge between the cusp coefficient estimate and the phase-uniform trapezoid lemma. The argument is mathematically equivalent to the monotonicity and domination step in Appendix 1, although it uses a positive integral mixture instead of the displayed binomial series.

**Exponential Fourier positivity.** The first part proves every real Fourier coefficient of `exp(t cos θ)` is nonnegative for `t≥0`. It constructs finite analytic and coanalytic exponential polynomials from the power series with nonnegative coefficients, verifies character orthogonality, and passes to the limit. Uniform bounds on the partial sums justify the convergence of their Fourier coefficients. This positivity is needed for the signed cosine coefficient of a fractional power; it is not an assumption about an unnamed Bessel function. The zero mode has different behavior, and the downstream domination theorem is stated for nonzero frequency.

**Smoothed cusp.** On `AddCircle 1`, `smoothedCuspBase δ` is the unit-circle form of `δ+1-cos(2πx)`. Its nonnegativity, periodic continuity, evenness, and behavior at `δ=0` are proved. The endpoint profile is identified with a constant multiple of the chord profile from `CircleFourier`; the factor is controlled by exact real-power identities. `smoothedCuspCosCoeff` is the real cosine coefficient. The module proves that the complex Fourier coefficient is real for this even function and equals that cosine coefficient. It also proves symmetry between frequencies `n` and `-n`, so the final positive-frequency estimate can be used on the whole integer lattice.

**Mixture and domination.** Apply `Schoenberg.schoenbergIntegral_eq_rpow_mul` with `p∈(0,1)` to represent each smoothed power as a positive mixture of `1-exp(-t r)`. The module proves the joint integrability needed to exchange the circle integral and the positive-half-line integral; this is the main technical step. At a nonzero Fourier frequency, the constant `1` vanishes, while the exponential term has nonnegative coefficient. Consequently the cusp coefficient is nonpositive. A pointwise comparison of the mixture weights at `δ≥0` and at `δ=0` then gives `|smoothedCuspCosCoeff p δ n|≤|smoothedCuspCosCoeff p 0 n|`. This includes `δ=0` as equality and avoids differentiating at the singular endpoint. Combining it with `CuspFourier`'s chord decay yields `exists_fourierCoeff_smoothedCuspCircle_power_bound`: one constant works for all `δ≥0` and every positive frequency.

**Range and limitations.** The proof requires `0<p<1`, corresponding exactly to `0<α<2` with `p=α/2`. The uniform constant depends on `α` only, not on `δ`, the phase, the future grid size, or geometric coefficients `A,B`. No Fourier inversion occurs here; it is supplied by `CuspTrapezoid`. The new scaffold's standalone `FourierDomination` and `FourierDecayBound` propositions use a real `[0,2π]` normalization, so this compiled circle result should be transferred explicitly before claiming those propositions proved. `AngularQuadrature` currently needs only the ported trapezoid theorem and does not make that extra identification.
