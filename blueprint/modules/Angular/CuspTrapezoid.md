# `BEMOCFormalization.Angular.CuspTrapezoid`
...
```lean
import BEMOCFormalization.Angular.CuspSmoothing
...
**Checked status.** This module builds after importing `Angular.CuspSmoothing`. It supplies the proved, phase-uniform trapezoid estimate for every `A≥B≥0`, including the equality cusp. It is an adaptation of the legacy generic module into `BEMOC.Definitive.Angular.CuspTrapezoid`; no old construction or population definition is imported. The final theorem `exists_uniform_cusp_trapezoid_all` is the analytic content of Lemma `trap` in `definitive.tex`, lines 445–458 and Appendix 1. Its constant is selected before `A`, `B`, the node count, or phase.

**Fourier aliasing.** For a continuous complex function on `AddCircle 1` with absolutely summable Fourier coefficients, the module proves a shifted root-grid identity. Pointwise Fourier inversion expresses the function at every shifted grid point as its Fourier series. Summing over `ZMod L` and using `CircleFourier.sum_fourier_rootPoint` kills frequencies not divisible by `L`. The remaining terms are the zero coefficient plus the sum over nonzero multiples `mL`, each multiplied by a character of the shift with norm one. The proof handles the infinite series through `HasSum` and the summability hypothesis; it does not infer pointwise equality merely from a coefficient bound. `L=1` is included by `[NeZero L]` and `1≤L`, with no special cancellation assumed.

**Tail estimate.** `CuspSmoothing` provides `|c_m(δ)|≤C_α |m|^(-1-α)` for all `δ≥0`, positive and negative nonzero integers. Since `α>0`, the positive power tail is summable. Reindexing the alias terms by integer multiples of `L` produces a factor `L^(-1-α)`. The two frequency signs contribute twice the positive tail; the phase character contributes no factor because its complex norm is one. This proves `exists_uniform_smoothedCusp_trapezoid` with a constant depending only on `α`, not on `δ`, `L`, or phase. The zero Fourier coefficient is proved equal to the normalized real integral over a full angular period, and the `ZMod` grid is converted to the ordinary real nodes `φ+2πk/L`.

**Rescaling to `A-B cos θ`.** For `B>0`, set `δ=(A-B)/B≥0` and `p=α/2`. The identity `A-B cos θ=B(δ+1-cos θ)` and nonnegative real-power multiplication give `(A-B cos θ)^p=B^p(δ+1-cos θ)^p`. The module proves this pointwise, for the finite sum, and for the integral. It transfers the normalized smoothed estimate and obtains `C_α B^p L^(-1-α)`. `A=B` means `δ=0`; this is already covered by the uniform smoothed theorem. The branch `B=0` is proved separately: both averages are the same constant `A^p`, so the error vanishes. This branch includes `A=B=0`. The hypotheses `0<α<2` supply both `p>0` for `0^p=0` and `p<1` for the smoothing theorem.

**Lean and manuscript checkpoints.** The final result uses `Finset.range L` and multiplier `1/L`; the current scaffold uses `Fin L` and `L⁻¹`, transferred in `AngularQuadrature`. The period normalization is `2π`, matching Appendix 1. The proof is independent of the gcd/lcm grid-multiplicity theorem, which is a later finite counting step for two rings. No population bound or `N` estimate occurs in this module. If changing the Fourier route, preserve absolute summability, both frequency signs, the arbitrary phase, the separate `B=0` branch, and the equality case `A=B`; omitting any of these weakens the manuscript lemma.
