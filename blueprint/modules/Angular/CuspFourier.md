# `BEMOCFormalization.Angular.CuspFourier`
...
```lean
import BEMOCFormalization.Angular.CircleFourier
...
**Checked status.** This module builds after importing `Angular.CircleFourier`. It adapts the legacy proof of the angular cusp Fourier decay into `BEMOC.Definitive.Angular`; it does not depend on the old geometric Core. The mathematical source is Appendix 1 of `definitive.tex`, especially the computation and `O(n^(-1-α))` estimate following `eq:f0-explicit`. The port proves the decay needed by later smoothing; the exact printed Gamma formula in the new scaffold's `CuspCoefficientFormula` is a separate statement and is not claimed here as an output theorem.

**Moment recurrence.** Define `sinePowerCosMoment α n = ∫₀^π sin(x)^α cos(2nx)dx`. The proof differentiates `sin(x)^(α+1)`, whose derivative extends continuously to both endpoints when `α>0`, and integrates by parts against an odd-frequency cosine. Elementary product-to-sum identities yield
`(α+2n+2) M(n+1)=(2n-α)M(n)`.
This recurrence is the formal substitute for the table beta integral used in Appendix 1. It avoids differentiating `sin(x)^α` at the endpoint. The module proves integrability and continuity first, then proves the recurrence using the interval integral integration-by-parts theorem. It converts the moments to the unit-period chord cosine coefficients by `x=πu` and verifies that sine coefficients vanish by reflection `u↦1-u`.

**Decay.** The recurrence is compared with the Gamma quotient model from `CircleFourier`. The initial coefficient fixes the scalar multiple; its exact value is immaterial for an upper bound. The Gamma model satisfies the same recurrence and has a nonzero first term. Its scaled limit gives a finite constant `C_α` with `|unitChordCosCoeff α n|≤C_α n^(-1-α)` for `n≥1`, provided `0<α<2`. The complex Fourier coefficient of the continuous circle chord profile equals the real cosine coefficient because of the proven reflection symmetry. This yields `exists_fourierCoeff_chordProfile_power_bound`, the key input to `CuspSmoothing`. The constant is quantified before frequency, and there is no smoothing parameter in this module.

**Endpoint and normalization checks.** `α=0` is excluded because the recurrence's endpoint regularity argument and cusp bound are formulated for positive exponent. `α=2` is excluded in the target theorem because the positive-frequency sign and `0<a<1` smoothing route require a strict upper bound. The low frequency `n=1` is included; `n=0` is not a decay target. The unit-period chord `(2 sin πu)^α` has a different scalar normalization from `(1-cos θ)^(α/2)`. The later `CuspSmoothing` module proves the exact bridge. The high-frequency coefficient exponent is `1+α`, which becomes `1+2a` in Appendix notation `a=α/2`. A bound of only `n^(-α)` would not suffice for the quadrature rate.
