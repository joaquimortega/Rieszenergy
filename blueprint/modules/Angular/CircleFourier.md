# `BEMOCFormalization.Angular.CircleFourier`
...
**Checked status.** The new module builds with Lean 4.19.0. It is adapted from the legacy `CircleFourier.lean` and deliberately lives in `BEMOC.Definitive.Angular.CircleFourier`, so none of its declarations collide with the older negative-type formalization. It imports Mathlib and its Fourier, Gamma, and circle APIs; it does not import the old Core or any configuration geometry. The module contains several historical circle Euler–Maclaurin definitions that are not needed for the present quadrature theorem. Their presence does not add axioms. The currently used chain is the unit-circle chord profile, Gamma-model asymptotics, Fourier coefficient identification, and root-grid character orthogonality.

**Mathematical role.** The chord profile `chordProfileCircle α` on `AddCircle 1` is the unit-period version of `(2 sin πx)^α`. This is the cusp profile that appears when Appendix 1 specializes `f₀(θ)=(1-cos θ)^(α/2)`; the two differ by an explicit power of two after `θ=2πx`. The definitions of `chordCoeffIntegral` and `chordFourierCoeffGamma` fix the complex Fourier normalization and the Gamma quotient. `tendsto_scaled_chordFourierCoeffGamma` controls the quotient at high frequency. `rootPoint`, `sum_fourier_rootPoint`, and the related divisibility forms establish that a regular grid annihilates Fourier characters whose integer frequencies are not divisible by the number of nodes. These identities are independent of cusp smoothing and of any ring population.

**Proof dependencies.** The Gamma-ratio limit uses `Real.Gamma_add_one`, nonvanishing and positivity at positive arguments, finite products, and asymptotic results already available in Mathlib. The chord-profile identity uses `AddCircle.toCircle`, the norm of the difference from one on the complex unit circle, and the trigonometric identity relating that norm to `2 sin πx` on a fundamental interval. Fourier coefficient identification uses integration on `AddCircle` and a change of variables to `[0,1]`. Root-grid orthogonality uses `ZMod w` and character sums; `[NeZero w]` is the Lean mechanism enforcing `w≥1`. The later `CuspFourier` and `CuspTrapezoid` modules import these checked statements. The import is a source dependency; the final analytic theorem is proved only after the subsequent modules connect coefficients to quadrature.

**Range and endpoint checks.** The geometric chord profile requires `0<α` for continuity at its zero. The desired Riesz range `0<α<2` enters later when the Gamma coefficient is negative and the Fourier tail has exponent strictly greater than one. The root-grid identity works for `w=1`; it makes no phase assumption. The zero Fourier mode must remain separate from positive and negative frequencies. This file does not claim Appendix 1's exact real coefficient formula by itself; `CuspFourier` supplies the recurrence and comparison that the quadrature proof uses. If the unused historical sections are later removed, preserve the named declarations referenced by `CuspFourier` and `CuspTrapezoid`, and rerun the module build before changing downstream imports.
