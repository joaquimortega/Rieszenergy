...
Checked progress: `diamondEnergy_nonneg` and `small_size_bound` are proved. The latter uses the explicit constant `(|continuousEnergy α|+1)*1024²` and `scale α N≥1` for α<2, uniformly in phases. The signed-band summation is proved in LatitudeSummation. Both unequal regimes are proved in UnequalBlockEstimates; `ComparableAssembly.comparable_block_bound` now discharges the comparable regime, so `latitude_bound_of_blocks` supplies the large-N latitude bound.
