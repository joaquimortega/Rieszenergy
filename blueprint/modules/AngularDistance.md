# Angular distance proof guide

Source anchors: `definitive.tex` latitude appendix, the chord-distance model used for comparable and separated bands. This module imports the proved `AngularGeometry` contract.

`polarChordSquare` is the exact squared chord in polar angles and a longitude difference. The half-angle identity gives

`D² = 4 sin²((φ−ψ)/2) + 4 sinφ sinψ sin²(θ/2)`.

For `φ,ψ∈[0,π]` and `|θ|≤π`, Jordan's bound and `|sin x|≤|x|` yield `x²/4≤4sin²(x/2)≤x²`. Since both polar sines are nonnegative,

`(1/4)((φ−ψ)²+sinφ sinψ θ²) ≤ D² ≤ (φ−ψ)²+sinφ sinψ θ²`.

`heightChordSquare_comparison` rewrites this model in height variables using `φ=arccos s`, `ψ=arccos t`, with radii `sqrt(1−s²)` and `sqrt(1−t²)`. `polarBand_near_angle_gap` gives `|φ−ψ|≤45/M` for points in occupied bands whose zero-based labels differ by at most two, by telescoping at most three angular widths. The direct height-band wrappers `band_radius_comparison`, `near_band_arccos_gap`, and `separated_band_arccos_gap` provide the radius and angle bounds needed by comparable and separated block arguments. These bounds do not themselves bound the kernel derivative or the longitude tail integral.

Status: all stated theorems are proved in `BEMOCFormalization/AngularDistance.lean`. Import the module transitively from the root when integrated.
