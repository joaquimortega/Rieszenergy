...
Status: the construction propositions are now inhabited by `constructionFacts : ConstructionFacts` and `boundaryFormulas : BoundaryFormulas`. The file also proves `boundary_width`, `card_pointIndex`, `height_in_open_unit`, `point_injective`, and the exact reflection identities. The canonical convention is implemented as `zeroPhases` and `diamondPoints`; `diamondPoints_card` proves cardinality `N`, and `diamondPoints_energy` reindexes the manuscript's ordered set energy to the labelled `diamondEnergy`. The total definitions still have arbitrary behavior outside occupied indices, so every manuscript correspondence retains `4≤N` and a valid `RingIndex`. The analytic energy bound for this set belongs to MainTheorem and remains separate from these construction proofs.
