# `BEMOCFormalization.DistanceScalarUniqueness` proof guide

A continuous function on `[-1,1]` is determined by all its moments against the Rodrigues Legendre polynomials. This module imports `DistanceKernelExpansion`, so its analytic transfer statements use exactly the geometric and spectral definitions in the main formalization.

The Rodrigues endpoint identity gives `P_ℓ` orthogonal to every polynomial of degree below `ℓ`. The checked leading-degree result in `DistanceZonal` gives a triangular spanning family: the linear span of all `P_ℓ` is the entire real polynomial ring. Integration against a continuous function is a linear functional on polynomials, hence vanishing Legendre moments imply vanishing moments for every polynomial.

To obtain pointwise uniqueness, approximate the continuous function uniformly by a polynomial using the Weierstrass theorem. The zero polynomial moments make its squared integral arbitrarily small; positivity then gives almost-everywhere zero, and continuity plus full support of interval Lebesgue measure gives zero everywhere. `continuousOn_eq_of_legendre_moments` applies this argument to a difference.

`intervalIntegral_tsum_of_summable_uniform_bound` justifies termwise integration of a uniformly absolutely summable series on the interval by domination of the integral norms. This is used for the distance-kernel Legendre series.
