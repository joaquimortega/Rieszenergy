...
Released under Apache 2.0 license as described in the file LICENSE-LeanReplay.
Adapted from Lean 4.19 Lean/Replay.lean for direct kernel replay.
-/
import Lean

namespace Comparator.KernelReplay
open Lean
namespace Replay
...
def addDecl (d : Declaration) : M Unit := do
  match Kernel.Environment.addDeclCore (← get).env 0 d (cancelTk? := none) with
...
open Replay
def replayKernel (newConstants : Std.HashMap Name ConstantInfo) (env : Kernel.Environment) : IO Kernel.Environment := do
