...
import ScratchComplexFinitePart
import ScratchZetaContinuation
import Mathlib.Analysis.Complex.LocallyUniformLimit
import Mathlib.Analysis.Complex.CauchyIntegral
import Mathlib.Analysis.Convex.Topology

/-!
# Alternative zeta identification by analytic continuation in the exponent

This scratch file isolates a route which avoids evaluating the explicit
real increment series against the zeta functional-equation coefficient.
...
theorem hasDerivAt_zetaPower0 (s : ℂ) {x : ℝ} (hx : 0 < x) :
  ...
  simpa only [zetaPower0, zetaPower1] using
    (ScratchZetaContinuation.hasDerivAt_powerFun (p := -s) hx)

theorem hasDerivAt_zetaPower1 (s : ℂ) {x : ℝ} (hx : 0 < x) :
    HasDerivAt (zetaPower1 s) (zetaPower2 s x) x := by
  convert (ScratchZetaContinuation.hasDerivAt_powerFun
    (p := -s - 1) hx).const_mul (-s) using 1 <;>
    simp only [zetaPower1, zetaPower2, ScratchZetaContinuation.powerFun] <;>
    ring_nf
...
theorem hasDerivAt_zetaPower2 (s : ℂ) {x : ℝ} (hx : 0 < x) :
  ...
  convert (ScratchZetaContinuation.hasDerivAt_powerFun
    ...
    simp only [zetaPower2, zetaPower3, ScratchZetaContinuation.powerFun] <;>
    ring_nf
...
theorem continuous_zetaPower3_on_Ici (s : ℂ) :
  ...
  exact ((ScratchZetaContinuation.hasDerivAt_powerFun
    (p := -s - 3) hx0).const_mul
  ...
  have hpoint : ∀ x ∈ Ι (((n + 1 : ℕ) : ℝ)) (((n + 2 : ℕ) : ℝ)),
    ...
    have hz3 : ‖zetaPower3 s x‖ ≤
      ...
      exact mul_le_mul hfac (hpow1.trans hpow2)
        (Real.rpow_nonneg _ _) (mul_nonneg hR0 (mul_nonneg (by linarith) (by linarith)))
    rw [norm_mul]
    calc
      ‖peanoKernelC ((n + 1 : ℕ) : ℝ) ((n + 2 : ℕ) : ℝ) x‖ *
          ‖zetaPower3 s x‖ ≤
